...
//gemeni api caller GEMINI API CALLER
...
//extract and validate structure of parse ads json
...
function parseAdsJson(raw) {
  try {
    ...
    // placement is optional — fill in a default if missing
...
app.post('/api/generate', async (req, res) => {
  try {
    ...
    return res.json({
      ...
      adsRaw: (!adsParsed?.ok && adsRaw) ? adsRaw : null, // fallback raw text if parse fails
      errors: {
...
app.listen(PORT, () => {
  console.log(`LaunchTrip démarré sur http://localhost:${PORT}`);
});
